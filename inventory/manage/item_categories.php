	...
	if ($input_error !=1)
	{
    	if ($selected_id != -1) 
    	{
		    update_item_category($selected_id, $_POST['description'],
				...
				check_value('no_sale'),$_POST['parent_category_id']);
    	...
    	else 
    	{
		    add_item_category($_POST['description'],
				...
				$_POST['dim2'],	check_value('no_sale'),$_POST['parent_category_id']);
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['no_sale']  = $myrow["dflt_no_sale"];
                $_POST['parent_category_id']  = $myrow["parent_category_id"];
	} 
...
/////adding custom cateogry parent
parent_cateogry_id_list_row(_("Parent Category:"), 'parent_category_id', null);
/////ending custom cateogry parent
