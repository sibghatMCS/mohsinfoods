...
{
       //print_r($_POST); exit;
   //    echo '<script> console.log("sibghat") </script>'; 
	$input_error = 0;
	if ($upload_file == 'No')
		$input_error = 1;
...
//print_r($_POST);
			...
			update_item($_POST['NewStockID'], $_POST['description'], $_POST['urdu_name'],
				$_POST['long_description'], $_POST['category_id'],$_POST['parent_category_id'] , 
		...
		{ //it is a NEW part
                  //  print_r($_POST); exit;
			add_item($_POST['NewStockID'], $_POST['description'],$_POST['urdu_name'],
				$_POST['long_description'], $_POST['category_id'],
                                $_POST['parent_category_id'] ,$_POST['tax_type_id'],
				$_POST['units'], $_POST['mb_flag'], $_POST['sales_account'],
				$_POST['inventory_account'], $_POST['cogs_account'],
				$_POST['adjustment_account'], $_POST['assembly_account'], 
				$_POST['dimension_id'], $_POST['dimension2_id'],
				check_value('no_sale'), check_value('editable'),check_value('not_sale'));
...
if (get_post('NewStockID') != get_post('stock_id') || get_post('addupdate')) { // first item display
			...
			$_POST['description'] = $myrow["description"];
                        $_POST['urdu_name']  = $myrow["urdu_name"];
...
//echo ' <input type="text" name="urdutest" id="tasktext"/>
//<input type="submit" value="teri maa ki " />    
//</form>';

	text_row(_("Name:"), 'description', null, 52, 200);
        text_row(_("Urdu Name:"), 'urdu_name', null, 52, 200,null,"","",'tasktext');
?>
...
setInterval(function(){
   makeUrduEditorsById({"tasktext":22});
	...
	stock_categories_list_row(_("Parent Category:"), 'category_id', null, false, $new_item);
...
parent_stock_categories_list_row(_("Cateroiges:"), 'parent_category_id', null, false, $new_item,$_POST['category_id']);
	...
	check_row(_("Exclude from sales:"), 'no_sale');
        check_row(_("Show on sale order:"), 'not_sale');
