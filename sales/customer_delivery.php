...
if (isset($_GET['AddedID'])) {
	...
	hyperlink_params_sib("$path_to_root/sales/customer_invoice.php", _("Invoice This Delivery"), "DeliveryNumber=$dispatch_no",true,'incov_link');
...
<script>
   $('document').ready(function () {
    ...
    window.location.href = "customer_invoice.php?DeliveryNumber=<?php echo $dispatch_no ?>&delivery=1";
               
      // $('#incov_link').click();
      //setTimeout(function(){ $('#incov_link').trigger('click');
    //alert('as dkjkasjd')  
    //},1000);
   // $('#invoice_load').load("customer_invoice.php?DeliveryNumber=<?php // echo $dispatch_no ?>")
      /*
           $.ajax({
            
type: "GET",
url: "customer_invoice.php?DeliveryNumber=<?php //echo $dispatch_no ?>",
success: function(data){
    alert(data)
//document.getElementById("hourrate").value = data;
			   }
			 });
...
if (isset($_POST['process_delivery']) && check_data() && check_qoh()) {
	...
	{
            close_sales_order($_POST['cusorderno']);
...
if (isset($_POST['Update']) || isset($_POST['_Location_update']) || isset($_POST['qty'])) {
 ...
 //  print_r($_POST); 
  ...
  //  echo '<script>alert("asdaksjdlk")</script>';
    //exit;
	$Ajax->activate('Items');
...
<style>
     ...
     .hide { display: none}
     .main { background-color: #D86464  !important; } 
</style>
<?php
start_form();
hidden('cart_id');
hidden('cusorderno', $_GET['OrderNumber']);
...
$th = array(_("Item Code"), _("Item Description"), 
	...
	_("This Delivery"), _("Price"), _("Tax Type"), _("Daig"), _("Total"));

table_header($th);
$k = 0;
$has_marked = false;
//print_r($_SESSION['Items']->line_items); //exit;
...
$sql="SELECT * FROM  `customer_order_detail` where sale_detail_id=$ln_itm->id ";
$res=  mysql_query($sql);

$rec=  mysql_fetch_array($res);

//print_r($rec['editable']);
if($rec['editable']==1)
    $cl="show $row_classes";
else
    $cl="hide $row_classes";
...
if($rec['main_order']==1)
	...
	$display_discount_percent = $ln_itm->id;//percent_format($ln_itm->discount_percent*100) . "%";

	$line_total = ($ln_itm->qty_dispatched * $ln_itm->price * (1 - $ln_itm->discount_percent));

	amount_cell($ln_itm->price);
	label_cell($ln_itm->tax_type_name);
        hidden('item_no'.$line,$ln_itm->id);
        hidden('total_no[]',$ln_itm->id);
        hidden('unit_price[]',$ln_itm->price);
...
{  $order_nos=$_GET['OrderNumber']; 

$_SESSION['order_no_sib']=$order_nos;
}
else {$order_nos=0;

if(isset($_SESSION['order_no_sib']))
{
    
    $order_nos=$_SESSION['order_no_sib'];
}

}
    $sql="SELECT * FROM  `customer_order` where sale_order = $order_nos ";
                        
                        $res=  mysql_query($sql);
                        $ssqle=  mysql_fetch_array($res);
//                        echo $_SESSION['updates'].'kasd klasjd lkasjd lkajsd laksjd'; 
                     
                     $totlaaa=$ssqle['itemcost'];
...
if (isset($_POST['Update']))
{
       $totlaaa=0; 
//    
//    echo count($_POST['total_no']);
$tot=0;   
    for($i=0; $i<count($_POST['total_no']); $i++)
    {
        $price=$_POST['unit_price'][$i];
$tot+=    $price*$_POST['Line'.$i];    
    }
            $_SESSION['itemcost']= $totlaaa;
//            echo  $tot;
            //exit;
              $totlaaa=$tot; 
...
start_row();
label_cell(_("Item Cost"), "colspan=$colspan align=right");
label_cell(_($totlaaa), "colspan=$colspan align=right");
end_row();


start_row();
label_cell(_("Packing Charges"), "colspan=$colspan align=right");
label_cell(_( $ssqle['packing_charges']), "colspan=$colspan align=right");
end_row();

start_row();
label_cell(_("Service Charges"), "colspan=$colspan align=right");
label_cell(_( $ssqle['service_charges']), "colspan=$colspan align=right");
end_row();

start_row();
label_cell(_("BBQ Charges"), "colspan=$colspan align=right");
label_cell(_( $ssqle['bbq_charges']), "colspan=$colspan align=right");
end_row();


start_row();
label_cell(_("Fare Charges"), "colspan=$colspan align=right");
label_cell(_( $ssqle['fare_charges']), "colspan=$colspan align=right");
end_row();
...
label_cell(_( $ssqle['discount']), "colspan=$colspan align=right");
end_row();



start_row();
label_cell(_("Advance"), "colspan=$colspan align=right");
label_cell(_( $ssqle['advance']), "colspan=$colspan align=right") ;
end_row();

$extr=$ssqle['fare_charges'] +$ssqle['service_charges'] + $ssqle['packing_charges'] +$ssqle['bbq_charges'];
          $balnvce=$totlaaa-($ssqle['discount']+$ssqle['advance']) + $ssqle['fare_charges'] +$ssqle['service_charges'] + $ssqle['packing_charges'] +$ssqle['bbq_charges'];

     start_row();
label_cell(_("Net Balance"), "colspan=$colspan align=right");
label_cell(_( $balnvce), "colspan=$colspan align=right");
end_row();                   
...
//                        start_row();
//label_cell(_("Advance Cost"), "colspan=$colspan align=right");
hidden('ChargeFreightCost', $extr);
