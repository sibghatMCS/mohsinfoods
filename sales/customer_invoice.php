...
//	display_note(print_document_link($invoice_no."-".$trans_type, _("&Print This Invoice"), true, ST_SALESINVOICE));
        ...
        display_note(print_document_link($_GET['deliveryid'], _("&Print Delivery Note"), true, ST_CUSTDELIVERY));
...
|| isset($_GET['BatchInvoice'])) {
    ...
    $_SESSION['deleid']=$_GET['DeliveryNumber'];
	processing_start();
	...
	foreach ($_SESSION['Items']->line_items as $line_no=>$itm) {
		if (isset($_POST['Line'.$line_no])) {
			...
			if (check_num('Line'.$line_no, $min, $max)) {
				...
				$ok = 0;
                           //     return $_POST['Line'.$line_no].'===='.$min.'--'.$max;
			}
...
function check_data()
{
	...
	if (!check_quantities()) {
		display_error(_(" asas as as as Selected quantity cannot be less than quantity credited nor more than quantity not invoiced yet."));
		return false;
	}
...
if (isset($_POST['process_invoice']) && check_data()) {
	...
	else
	{
		...
		if ($newinvoice) {
                   $did= $_POST['delevry_no'];
			meta_forward($_SERVER['PHP_SELF'], "AddedID=$invoice_no&deliveryid=$did");
		} else {
...
hidden('delevry_no', $_GET['DeliveryNumber']);
