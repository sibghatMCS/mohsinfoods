...
function print_aged_customer_analysis()
{
	...
	while ($myrow=db_fetch($result))
	{
            ...
            $sql="SELECT * FROM  `0_crm_persons` where ref='".$myrow['debtor_ref']."' ";
            $result1 = db_query($sql, "The customers could not be retrieved");

	$myrow_add=db_fetch($result1);
		...
		$rep->fontSize += 2;
		$rep->TextCol(0, 2, $myrow['name'].' '.$myrow_add['address']);
//		if ($convert) $rep->TextCol(2, 3,	$myrow['curr_code']);
                    $rep->TextCol(2, 3,	$myrow_add['phone']);
                $rep->fontSize -= 2;
